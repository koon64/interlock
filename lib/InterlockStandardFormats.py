...
class InterlockSong:
    def __init__(self, url, title, image, artist=None, album=None, play_music_id=None, play_music_class=None,
                 duration_seconds=0, album_position=0):
        self.url = url
        self.title = title
        self.image = image
        self.artist = artist
    ...
    def __str__(self):
        ...
        if self.album is not None:
            alb = " | " + self.album
        return "[ INTERLOCK SONG | "+self.artist+" | "+self.title+alb+" | "+duration+" ]"
...
class InterlockAlbum:
    ...
    def add_song(self, song, dont_sort_songs=False):
        if type(song) is InterlockSong:
            self.songs.append(song)
            if not dont_sort_songs:
                self.order_songs()
    ...
    def order_songs(self):
        self.songs = sorted(self.songs, key=lambda song: song.album_position)
